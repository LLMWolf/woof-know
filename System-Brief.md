...
Правила: OpenAPI 3.1; ошибки — RFC7807; пагинация курсорная где нужен листинг.
...
| NFR                               | Тактики                                                                                                                  |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Производительность (P95 ≤ 7–10 c) | SQLite WAL; индексы; кэш кандидатов; ограничение `k` и `max_tokens`; преформат промптов; батчинг LLM вызовов (локально). |
...
| Масштабируемость                  | Горизонталь по процессам API/ingest; sharding по рабочим каталогам; вынесение LLM на отдельный узел (локальный).         |
| Наблюдаемость                     | OpenTelemetry метрики/трейсы; event-лог Q&A; KPI-дашборд (P95, hit@10, no-answer%, net-positive).                        |
| Стоимость                         | Локальный LLM (нулевой переменный счёт); SQLite (без сервера); режим гибрид/облако — опционально, с бюджетными лимитами. |
| ДР                                | Резервные копии DB+FS; quarterly restore test; “expand–migrate–contract” миграции.                                       |
| i18n/a11y                         | **RU-only** UI до R2; WCAG 2.1 AA; позже — стратегия i18n.                                                               |
...
* **R0.1 (CLI, 2–3 недели):** ingest md/txt; локальный Q&A; отчёты; конфиг; логи/KPI сбор; офлайн-набор Q&A — **T0+7**.
